...
func ForwardGetListOrganization(userId string) (*http.Response, error) {
	cfg := config.LoadConfig()

	registrationUrl := fmt.Sprintf("%s/org/get-list", cfg.BackendURL)
	contentType := "application/json"

	// Create a new HTTP request with the JSON data
	req, err := http.NewRequest("GET", registrationUrl, nil)
	if err != nil {
		log.Println("ForwardGetListOrganization | Create request error: ", err)
	...
	if err != nil {
		log.Println("ForwardGetListOrganization | Send request error: ", err)
